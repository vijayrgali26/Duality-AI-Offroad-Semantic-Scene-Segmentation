...
def main():
    parser = argparse.ArgumentParser(description='Train a DINOv2-based segmentation head on the Offroad dataset')
    parser.add_argument('--data_root', type=str,
                        default=os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'Offroad_Segmentation_Training_Dataset'),
                        help='Dataset root containing train/ and val/')
    parser.add_argument('--output_dir', type=str, default=None,
                        help='Directory for training logs and plots')
    parser.add_argument('--batch_size', type=int, default=2, help='Batch size')
    parser.add_argument('--epochs', type=int, default=10, help='Number of training epochs')
    parser.add_argument('--lr', type=float, default=1e-4, help='Learning rate')
    parser.add_argument('--img_width', type=int, default=480, help='Resize width for training images')
    parser.add_argument('--img_height', type=int, default=270, help='Resize height for training images')
    ...
    w = int((args.img_width // 14) * 14)
    h = int((args.img_height // 14) * 14)
    lr = args.lr
    n_epochs = args.epochs

    script_dir = os.path.dirname(os.path.abspath(__file__))
    output_dir = args.output_dir or os.path.join(script_dir, 'train_stats')
    os.makedirs(output_dir, exist_ok=True)
    ...
    train_loader = DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=args.num_workers)
    valset = MaskDataset(data_dir=val_dir, transform=transform, mask_transform=mask_transform)
    val_loader = DataLoader(valset, batch_size=batch_size, shuffle=False, num_workers=args.num_workers)
